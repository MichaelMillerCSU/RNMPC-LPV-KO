...
deltaT = 0.05;
liftFun = @(xx)( [xx]);
% liftFun = @(xx)( [xx; xx(1).*xx(2); xx(1).^2]);
...
%     if i > 1
%         [V,D] = eig(A{2});
%         Q_A = 4 * rand(Nlift, Nlift) - 2;
%         etaA = 1;
%         etaB = 1;
% %         A{i} = etaA .* (A{2} + 0.02 * rand(Nlift, Nlift) - 0.01);
% %         B{i} = etaB .* (B{2} + 0.02 * rand(Nlift, 1) - 0.01);
%         A{i} = etaA .* (A{2} + 0.1 * rand(Nlift, Nlift) - 0.05);
%         B{i} = etaB .* (B{2} + 0.1 * rand(Nlift, 1) - 0.05);
%         Eig{i} = eig(A{i});
%         Eig_max = [Eig_max max(Eig{i})];
%     end
% end
A{end + 1} = [1e-6* rand(Nlift, Nlift)];
...
for i = 1 : 1
    ...
    Net_Vertices{i}.layers{1}.transferFcn = 'tansig'; % 
